...
Window Mainpanel() : Panel
	...
	Button button7 title=" New Map",font="Times New Roman",fSize=20, pos={100,315},size={100,30},proc=ButtonProc_NewMap,fstyle=1,fColor=(0,0,65535)
...
Function ButtonProc_dataexport(ctrlName) : ButtonControl
   ...
   DoUpdate/W=Loadprogress/E=1
	if (stringmatch(datatype,".txt")==1 && V_value!=6)
		for(i=0; i<index; i+=1)
			if(dataselwave[i]!=0)
				k+=1
...
Function ButtonProc_cuttranspose(ctrlName) : ButtonControl
	String ctrlName
	String wavestr
	variable xoff, xdelta, yoff, ydelta, xsize, ysize
	...
	for(i = 0; i < size; i += 1)
		if(dataselwave[i] != 0)
			...
			if(waveexists(currentwave) != 1)
				...
				DeletePoints/M=1 point2, columnsize-point2-1, currentwave
				DeletePoints/M=1 0, point1-1, currentwave
				setscale/P y, offset, delta, "", currentwave
...
Window NewImagePanel() : Graph
	...
	Display/W=(400,100,900,700) /N=NewImagewindow
	ControlBar 70
   ...
   Label EDCaxisB "\\F'Times New Roman'\\Z24\f00 EDC Intensity"
   Label MDCaxisL "\\F'Times New Roman'\\Z24\f00 MDC Intensity"
   ...
   SetVariable setvar6 title="Num",limits={1,inf,1},font="Times New Roman",fSize=14,fstyle=1
	...
	PopupMenu popup0,font="Times New Roman",fSize=20,fStyle=1
...
Function ButtonProcScope(ctrlName) : ButtonControl
	...
	if(V_flag==0)
		...
		ListBox list0 pos={10,120},size={180,460}, mode=9, font="Times New Roman",fSize=16
...
Window NewMapPanel() : Graph
	...
	Display/W=(400,100,900,700) /N=NewMapwindow
	ControlBar 70
...
// brief help document for 3D Map window
	...
	if(V_flag==0)
		...
		logtext+="Arrow Keys: increase/decrease the X or Y value by deltaZ, move the lines on X-Y iamge and live update the images and curve;\r\r"
...
Function ButtonProc3DMapAzimuth()
	...
	if(V_flag==0)
		...
		controlBar 50
		Slider Azislider,pos={10.00,1.00},size={350.00,52.00},proc=AzimuthGraphSliderProc,win=NewAzimuthGraph
		Slider Azislider,font="Times New Roman",fSize=10
		Slider Azislider,limits={-90,90,0},value= 0,side= 2,vert= 0,ticks= 6
		SetVariable setAzislidevar,pos={364.00,14.00},size={61.00,22.00},proc=SetAzisliderVarProc,win=NewAzimuthGraph
		SetVariable setAzislidevar,font="Times New Roman",fSize=16,title=" "
		SetVariable setAzislidevar,limits={-90,90,0},value= _NUM:0
		
		AppendImage/W=NewAzimuthGraph Azimuthwave
		
		ModifyGraph width=256,fsize=20
...
Function ButtonProc3DMapNewGraph(ctrlName) : ButtonControl
	...
	if(stringmatch(graphwave,"zintensity")==1)
		...
		Label/Z bottom "\\F'Times New Roman'\\Z24\f02E-E\BF\M\F'Times New Roman'\\Z24\f00 (eV)"
   		...
   		TextBox/C/N=text0/B=1/F=0/A=MC "\\F'Times New Roman'\\Z24zintensity"
	elseif(stringmatch(graphwave,"currentzwave")==1)
		...
		exportgraphplot()
		ModifyGraph zero=4
		Label/Z left "\\F'Times New Roman'\\Z24\f00k\By\M\F'Times New Roman'\\Z24"
		Label/Z bottom "\\F'Times New Roman'\\Z24\f00k\Bx\M\F'Times New Roman'\\Z24"
	...
	elseif(stringmatch(graphwave,"currentywave")==1)
		...
		ModifyGraph zero=4
		Label/Z left "\\F'Times New Roman'\\Z24\f02E-E\BF\M\F'Times New Roman'\\Z24\f00 (eV)"
		Label/Z bottom "\\F'Times New Roman'\\Z24\f00k\By\M\F'Times New Roman'\\Z24"
		ModifyGraph margin(left)=70,margin(bottom)=56,margin(right)=28,margin(top)=28,width=269.291,height=340.157
		if(mapcolorcheck==1)
			 PauseUpdate; Silent 1		// modifying window...
   			 ModifyImage/Z [0] ctab= {*,*,NewMapcolortab ,1}
...
Function Buttonproc_FSMapRotation (ctrlName) : ButtonControl
   ...
   termY1=-sin(Psi)*cos(Theta)-cos(Phi)*sin(Theta)*cos(Psi)
	...
	if(FSapicheck==1)
		...
		Label left "\\F'Times New Roman'\\Z24\\f00 k\\By\\M\\F'Times New Roman'\\Z24 (Å\\S-1\\M\\F'Times New Roman'\\Z24)"
	else
		Label bottom "\\F'Times New Roman'\\Z24\\f00 k\\Bx\\M\\F'Times New Roman'\\Z24 (π/a)"
		Label left "\\F'Times New Roman'\\Z24\\f00 k\\By\\M\\F'Times New Roman'\\Z24 (π/a)"
	endif
End

Function ButtonProc_FSexport(ctrlName) : ButtonControl
	String ctrlName
	wave FSrotateMap, FSrotatemapcolortab
	string/g FSmapname
	variable/g FSenergy, FSinvertcheck, FSapicheck
	String newFSname
	if(waveexists(FSrotateMap)==1)
		...
		Display;Delayupdate 
  		...
  		ModifyGraph margin(left)=75,margin(bottom)=75,margin(right)=28,margin(top)=28,width=368.504,height={Aspect,1}
		ModifyGraph mirror=1,fStyle=1,fSize=16,font="Times New Roman"
		ModifyGraph standoff=0, tick=2, zero=4,zeroThick=2
		ModifyGraph axThick=2
...
Function ButtonProc_PiAngleCal (ctrlName) : ButtonControl
   ...
   variable AngAtPi, AngAtHalfPi, AngAtsqrt2Pi,AngAt2sqrt2Pi, AngAt2Pi
	...
	dowindow/f EvsKz
	if (V_flag!=1)
		edit Energy_eV, HalfPiangle_deg, Piangle_deg, kz_AtZero_pi, kz_AtPi_pi, kz_AtSqrtPi_pi, kz_At2Pi_pi, kz_At2SqrtPi_pi
 		dowindow/c EvsKz
	endif
	if(stringmatch(plotcheck,"Yes")==1)
		...
		if(V_flag!=1)
			...
			Label left "\\K(65535,0,0)\\F'Times New Roman'\\Z24 Pi angle (°)"
			Label bottom "\\F'Times New Roman'\\Z24\\f00 hν (eV)"
			Label right "\\K(0,0,65535)\\F'Times New Roman'\\Z24 k\\Bz\\M\\F'Times New Roman'\\Z24 (π/c)"
		endif
	endif
...
Function ButtonProc_ImAxisCorrFunclaser (ctrlName) : ButtonControl
	...
	if (V_flag!=1)
		...
		ColorScale/C/N=text0/F=0/A=MC/X=58.00/Y=5.00 nticks=0
		Label/Z left "\\F'Times New Roman'\\Z24\f02E-E\BF\M\F'Times New Roman'\\Z24\f00 (eV)"
		Label bottom "\\F'Times New Roman'\\Z24\\f00k (Å\\S-1\\M\\F'Times New Roman'\\Z24)"
	endif
End
...
Proc Buttonproc_KzmapCorr (ctrlName) : ButtonControl
	...
	if (V_flag!=1)
		...
		ModifyGraph minor=1
		ModifyGraph/Z font="Times New Roman", fSize=16
		ModifyGraph axThick=2 
		ModifyGraph standoff=0
		ModifyGraph zero(bottom)=4,zeroThick(bottom)=2
		Label left "\\F'Times New Roman'\\Z24\f00Photon energy (eV)"
		Label bottom "\\F'Times New Roman'\\Z24\f00MCP angle (deg)"
	...
	if (V_flag!=1)
		...
		ModifyGraph standoff=0
		ModifyGraph zero(bottom)=4,zeroThick(bottom)=2
		Label bottom "\\F'Times New Roman'\\Z24\f00k\B//\M\F'Times New Roman'\\Z24 (π/a)"
		Label left "\\F'Times New Roman'\\Z24\f00k\Bz\M\F'Times New Roman'\\Z24 (π/c)"
...
Function Buttonproc_3DKzmapCorr (ctrlName) : ButtonControl
   ...
   DoUpdate/W=kzRotationprogress/E=1
	...
	//SetScale/I y photonenergy,photonenergy+Epoints*kzhvstep/kzInterpY,"", kzSourceMap
	killwaves kzSourceMap1, ThetaMap1, Ekmap1, kzcubicmapint, temp1, temp2
...
Function ButtonProc_kzmapexport(ctrlName) : ButtonControl
	...
	string/g  kzmapname
	variable/g kzFSenergy, kzinvertcheck
	string newkzmapname
	if(waveexists(kzMap)!=1)
	...
	ModifyGraph/Z font="Times New Roman",fSize=16, standoff=0
	ModifyGraph axThick=2
...
Function ButtonProc_1Dtracesymmetry(ctrlName) : ButtonControl
	...
	ModifyGraph tick=2,mirror=1,fSize=16,standoff=0,fStyle=1,lsize=2,axThick=2,font="Times New Roman"
...
Function DrawImageplot()
	...
	ModifyGraph axThick=2
	Label left "\\F'Times New Roman'\\Z24\\f02E-E\\BF\\M\\F'Times New Roman'\\Z24\\f00 (eV)"
	Label bottom "\\F'Times New Roman'\\Z24 k\\B//\\M\\F'Times New Roman'\\Z24 (Å\\S-1\\M\\F'Times New Roman'\\Z24)"
...
Function MDCDispplot()
	...
	Modifygraph rgb(MDCDispE)=(0,0,0), mode(MDCDispE)=3, marker(MDCDispE)=8, msize(MDCDispE)=4, mrkThick(MDCDispE)=1.5
	 ...
	 Label/Z left "\\F'Times New Roman'\\Z24\f01E-E\BF\M\F'Times New Roman'\\Z24\f00(eV)"
	 Label/Z bottom "\\F'Times New Roman'\\Z24\f00k\F'Times New Roman'\\Z24 (Å\S-1\M\F'Times New Roman'\\Z24)"
End
...
Function ButtonProc_selfenergyload(ctrlName) : ButtonControl
	...
	if(V_flag==0)
		...
		ModifyGraph mirror=1,fStyle=1,fSize=16,font="Times New Roman"
		Label bottom "\\F'Times New Roman'\\Z24\\f00 k\\B//\\M\\F'Times New Roman'\\Z24(Å\\S-1\\M\\F'Times New Roman'\\Z24)"
		Label left "\\F'Times New Roman'\\Z24\\f02E-E\\BF\\M\\F'Times New Roman'\\Z24\\f00 (eV)"
	endif
